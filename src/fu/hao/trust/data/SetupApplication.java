...
public class SetupApplication {
	...
	public void calculateSourcesSinksEntrypoints(
		...
		// Create the SourceSinkManager
		{
			...
			sourceSinkManager = new AccessPathBasedSourceSinkManager(
					...
					config.getLayoutMatchingMode(), lfp == null ? null
							: lfp.getUserControlsByID());

			sourceSinkManager.setAppPackageName(this.appPackageName);
			sourceSinkManager.setResourcePackages(this.resourcePackages);
			sourceSinkManager.setEnableCallbackSources(this.config
					.getEnableCallbackSources());
		}
	...
	/**
	 * @Title: loadClassesAndCG
	 * @Description: TODO
	 * @param
	 * @return void
	 * @throws
	 */
	public void loadClassesAndCG() {
		System.out.println("Retriving call graph and icfg on "
				+ apkFileLocation);
		Infoflow info;
		if (cfgFactory == null)
		...
		info.initializeSoot(apkFileLocation, path,
				entryPointCreator.getRequiredClasses());
		...
		info.constructCG(sourceSinkManager);
		logger.info("cgsa" + Scene.v().getCallGraph().size());
	}
	...
	private String getClasspath() {
		String classpath = forceAndroidJar ? androidJar : Scene.v()
				.getAndroidJarPath(androidJar, apkFileLocation);
		if (this.additionalClasspath != null
				&& !this.additionalClasspath.isEmpty())
	...
	public InfoflowResults runInfoflow(
		...
		// sourceSinkManager);
		info.computeInfoflow(sourceSinkManager);
